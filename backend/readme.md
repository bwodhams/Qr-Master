...
| POST | {HOST}/api/user/ getQRCodes | Get user's generated QRCodes | Body containing loginAuthToken | JSON object `{"message": "some message", "qrcodes": "array of all generated qrcodes for user"}` | /api/user/getQRCodes with body of `{"loginAuthToken": "userLoginAuthToken"}` |
| POST | {HOST}/api/user/ deleteQRCode | Delete a QR code | Body containing loginAuthToken, deleteID | JSON object `{"message": "some message"}` | /api/user/deleteQRCode with body of `{"loginAuthToken": "userLoginAuthToken", "deleteID": 0}` |
